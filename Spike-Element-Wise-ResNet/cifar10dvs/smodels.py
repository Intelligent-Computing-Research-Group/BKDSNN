...
class ResNetN(nn.Module):
    def __init__(self, layer_list, num_classes, connect_f=None, indices=(9,)):
    ...
    def forward(self, x: torch.Tensor, return_inter=False):
        ...
        if return_inter:
            ...
            for i, layer in enumerate(self.conv):
                ...
                if i in self.indices:
                    output.append(x.mean(0))
            output.append(self.out(x.mean(0)))
            return self.out(x.mean(0)), output
        else:
            x = self.conv(x)
            return self.out(x.mean(0))

def SEWResNet(connect_f, indices=(9,)):
    layer_list = [
    ...
    num_classes = 10
    return ResNetN(layer_list, num_classes, connect_f, indices=indices)

if __name__ == '__main__':
    model = SEWResNet("ADD").cuda()
    # model.load_state_dict(torch.load("ckpt/best.pth", map_location="cpu")["net"])
    input = torch.randn((16,16,2,128,128)).cuda()
    _, output = model(input, True)
