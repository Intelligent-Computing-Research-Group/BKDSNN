...
class SEWResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, T=4, connect_f=None, indices=(3,)):
    ...
    def _forward_impl(self, x, return_inter=False):
        ...
        if return_inter:
            ...
            for i, blk in enumerate(self.layer1):
                x, outs = blk(x, return_inter)
                if 0 in self.indices and i == len(self.layer1) - 1:
                    intermeidate.append(x.mean(0))
                    # intermeidate.append(outs)
            for i, blk in enumerate(self.layer2):
                x, outs = blk(x, return_inter)
                if 1 in self.indices and i == len(self.layer2) - 1:
                    intermeidate.append(x.mean(0))
                    # intermeidate.append(outs)
            for i, blk in enumerate(self.layer3):
                x, outs = blk(x, return_inter)
                if 2 in self.indices and i == len(self.layer3) - 1:
                    intermeidate.append(x.mean(0))
                    # intermeidate.append(outs)
            for i, blk in enumerate(self.layer4):
                x, outs = blk(x, return_inter)
                if 3 in self.indices and i == len(self.layer4)-1:
                    intermeidate.append(x.mean(0))
                    # intermeidate.append(outs)

            x = self.avgpool(x)
            intermeidate.append(x.mean(dim=0))
            x = torch.flatten(x, 2)
            intermeidate.append(self.fc(x.mean(dim=0)))
            return self.fc(x.mean(dim=0)), intermeidate
        else:
            x = self.layer1(x)
            x = self.layer2(x)
            x = self.layer3(x)
...
if __name__ == '__main__':
    model = sew_resnet18(zero_init_residual=True, connect_f="ADD")
    print(model)