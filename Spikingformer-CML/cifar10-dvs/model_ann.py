...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_lif = nn.ReLU(inplace=True)
        ...
        self.proj2_lif = nn.ReLU(inplace=True)
        ...
        self.proj3_lif = nn.ReLU(inplace=True)
        ...
        self.proj4_lif = nn.ReLU(inplace=True)
...
def Spikingformer(pretrained=False, **kwargs):
    model = vit_snn(
        patch_size=8, embed_dims=512, num_heads=16, mlp_ratios=4,
...
if __name__ == '__main__':
    ...
    model = VisionTransformer(img_size=128, in_chans=2, num_classes=10).cuda()
    input = torch.randn(4,16,2,128,128).cuda()

    print(input.flatten(0,1).size())
    output = model(input.flatten(0,1))
    print(output.size())
