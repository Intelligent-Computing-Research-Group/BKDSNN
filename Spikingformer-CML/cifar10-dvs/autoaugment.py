...
class SNNAugmentWide(torch.nn.Module):
    ...
    def __init__(self, num_magnitude_bins: int = 31, interpolation: InterpolationMode = InterpolationMode.NEAREST,
                 fill: Optional[List[float]] = None, img_size_h=128, img_size_w=128) -> None:
        super().__init__()
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        if img.shape[-2:] != (self.img_size_h, self.img_size_w):
            # print(img.shape)
            img = torch.nn.functional.interpolate(img, (self.img_size_h, self.img_size_w), mode="nearest")
