...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/data/zkxu/cifar-10-dvs/', help='dataset')
    ...
    parser.add_argument('--T_teacher', default=16, type=int, help='simulation steps')
    ...
    #                     help='number classes')
    parser.add_argument('--distill_weight', default=0.1, type=float, metavar='DW',
                        help='distill weight')
    ...
    parser.add_argument('--teacher_channel', default=256, type=int, metavar='TC',
                        help='teacher_emb')
    parser.add_argument('--student_channel', default=256, type=int, metavar='SC',
                        help='student_emb')
...
def train_one_epoch(model, model_teacher, criterion, dist_loss, optimizer, data_loader, device, epoch, print_freq, args, scaler=None, T_train=None, aug=None, trival_aug=None, mixup_fn=None):
    model.train()
    model_teacher.eval()
    ...
    for image, image_teacher, target in metric_logger.log_every(data_loader, print_freq, header):
        start_time = time.time()
        image, image_teacher, target = image.to(device), image_teacher.to(device),target.to(device)
        image = image.float()  # [N, T, C, H, W]
        image_teacher = image_teacher.float()  # [N, T, C, H, W]
        ...
        if aug != None:
            ...
            image_teacher = torch.stack([(aug(image_teacher[i])) for i in range(N)])
            # image = image.reshape(N,T,C,H,W)

        if trival_aug != None:
            # image = image.flatten(0,1).contiguous()
            image = torch.stack([(trival_aug(image[i])) for i in range(N)])
            image_teacher = torch.stack([(trival_aug(image_teacher[i])) for i in range(N)])
        ...
        if scaler is not None:
            with amp.autocast():
                out_fr, out_feature_s = model(image, True)
                with torch.no_grad():
                    _, out_feature_t = model_teacher(image_teacher, True)
                if args.distill_type != "mixed":
                    if args.distill_type == "wsld":
                        loss_dist = args.distill_weight * get_logits_loss(out_feature_t[-1], out_feature_s[-1],
                                                                          target, args.temp, 10)
                    elif args.distill_type == "lasnn":
                        loss_dist = dist_loss(out_feature_t[:len(args.indices)], out_feature_s[:len(args.indices)])
                    else:
                        loss_dist = dist_loss(out_feature_t[len(args.indices)-1], out_feature_s[len(args.indices)-1])
                else:
                ...
                loss_ce = criterion(out_fr, target)
        ...
        if mixup_fn is not None:
            acc1, acc5 = utils.accuracy(out_fr, target_for_compu_acc, topk=(1, 5))
        else:
            acc1, acc5 = utils.accuracy(out_fr, target, topk=(1, 5))
        ...
        metric_logger.update(loss=loss_s, lr=optimizer.param_groups[0]["lr"], loss_ce=loss_ce, loss_dist=loss_dist)
        if args.distill_type == "mixed":
            metric_logger.update(loss_dist_feat=loss_dist_feat, loss_dist_logits=loss_dist_logits)
    ...
    if args.distill_type == "mixed":
        return metric_logger.loss.global_avg, metric_logger.loss_ce.global_avg, metric_logger.loss_dist.global_avg, metric_logger.loss_dist_feat.global_avg, metric_logger.loss_dist_logits.global_avg, metric_logger.acc1.global_avg, metric_logger.acc5.global_avg
    else:
        return metric_logger.loss.global_avg, metric_logger.loss_ce.global_avg, metric_logger.loss_dist.global_avg, metric_logger.acc1.global_avg, metric_logger.acc5.global_avg
...
def evaluate(model, criterion, data_loader, device, print_freq=100, header='Test:'):
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    with torch.no_grad():
        for image, _, target in metric_logger.log_every(data_loader, print_freq, header):
...
def main(args):
    ...
    output_dir = os.path.join(args.output_dir, f'{args.model}_b{args.batch_size}_T{args.T}_{args.T_teacher}')
    ...
    output_dir += '_{}'.format(args.distill_type)
    if not os.path.exists(output_dir):
        utils.mkdir(output_dir)
    ...
    if args.distill_type == 'bkd' or args.distill_type == 'mixed':
        dist_loss = BRDLoss(teacher_emb=args.teacher_channel, student_emb=args.student_channel,
                         lambda_mgd=args.lambda_mgd, alpha_mgd=args.alpha_mgd, use_clip=args.use_clip).to(args.device)
    elif args.distill_type == "kdsnn":
        dist_loss = KDSNNLoss(teacher_emb=args.teacher_channel, student_emb=args.student_channel,
                           alpha_mgd=args.alpha_mgd).to(args.device)
    elif args.distill_type == "lasnn":
        dist_loss = LaSNNLoss(teacher_emb=args.teacher_channel, student_emb=args.student_channel,
    ...
    model_teacher.load_state_dict(torch.load("logs/best.pth")["model"])
    ...
    if args.distill_type != "wsld":
        optimizer = create_optimizer(args, nn.ModuleList([model, dist_loss]))
    else:
    ...
    for epoch in range(args.start_epoch, num_epochs):
        ...
        if args.distill_type == "mixed":
            train_loss, train_loss_ce, train_loss_dist, train_loss_dist_feat, train_loss_dist_logits, train_acc1, train_acc5 = train_one_epoch(
                model, model_teacher, criterion_train, dist_loss, optimizer, data_loader, device, epoch,
                args.print_freq, args, scaler, args.T_train,
                train_snn_aug, train_trivalaug, mixup_fn)
        else:
            train_loss, train_loss_ce, train_loss_dist, train_acc1, train_acc5 = train_one_epoch(
                model, model_teacher, criterion_train, dist_loss, optimizer, data_loader, device, epoch,
                args.print_freq, args, scaler, args.T_train,
                train_snn_aug, train_trivalaug, mixup_fn)
        if utils.is_main_process():
            ...
            train_tb_writer.add_scalar('train_loss_dist', train_loss_dist, epoch)
            if args.distill_type == "mixed":
                train_tb_writer.add_scalar('train_loss_dist_feat', train_loss_dist_feat, epoch)
                train_tb_writer.add_scalar('train_loss_dist_logits', train_loss_dist_logits, epoch)
