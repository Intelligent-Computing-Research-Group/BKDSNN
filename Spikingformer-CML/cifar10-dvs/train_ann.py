...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/nvme/cifar-10-dvs/', help='dataset')
    ...
    parser.add_argument('--img_size_h', type=int, default=128, metavar='N',
                        help='number of epochs to train (default: 2)')
    parser.add_argument('--img_size_w', type=int, default=128, metavar='N',
                        help='number of epochs to train (default: 2)')
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if scaler is not None:
            with amp.autocast():
                output = model(image)
                if args.model != "spikingformer":
                    output = output.reshape(N,T,-1).mean(1)
...
def main(args):
    ...
    if args.model == "spikingformer":
        model = create_model(
            ...
            img_size_h=args.img_size_h, img_size_w=args.img_size_w
        )
    else:
        model = VisionTransformer(img_size=args.img_size_h, in_chans=2, num_classes=10, num_heads=16)
    ...
    train_trivalaug = autoaugment.SNNAugmentWide(img_size_h=args.img_size_h, img_size_w=args.img_size_w)
    mixup_fn = None
