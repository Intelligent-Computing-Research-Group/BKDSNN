...
class vit_ann(nn.Module):
    def __init__(self,
                 img_size_h=128, img_size_w=128, patch_size=16, in_channels=2, num_classes=11,
                 embed_dims=[64, 128, 256], num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=1, pretrained_cfg= None, pretrained_cfg_overlay= None
                 ):
        super().__init__()
    ...
    def forward_features(self, x, return_inter=False):
        ...
        for blk in block:
            x = blk(x)
        if return_inter:
            return x.flatten(3).mean(3), x.mean(0)
        else:
    ...
    def forward(self, x, return_inter=False):
        ...
        if return_inter:
            x, out = self.forward_features(x, return_inter)
            x = self.head(x.mean(0))
            return x, [out, x]
        else:
            x = self.forward_features(x, return_inter)
            out = x.mean(0)
            x = self.head(x.mean(0))
            return x
...
@register_model
def ANNTransformer(pretrained=False, **kwargs):
    model = vit_ann(
        **kwargs
    )
