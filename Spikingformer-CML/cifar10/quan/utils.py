    ...
    for name, module in model.named_modules():
        if type(module) in QuanModuleMapping.keys():
            if name in quan_scheduler.excepts:
                # print(name, quantizer(quan_scheduler.weight, quan_scheduler.excepts[name].weight), quantizer(quan_scheduler.act, quan_scheduler.excepts[name].act))
